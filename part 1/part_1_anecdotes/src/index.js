...
const App = () => {
    ...
    const [mostVotedAnecdote, setMostVoted] = useState(anecdotes.get(selected))
    ...
    function SetMostVotedAnecdote() {
        var mostVoted = 0
        var topAnecdote = mostVotedAnecdote
        state.forEach((item) => {
            if (item.votes > mostVoted) {
                mostVoted = item.votes
                topAnecdote = item
            }
        })
        setMostVoted(topAnecdote)
        return (
            <>
